...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (!pairVia12) {
		...
		//pass[0] = radioInfo.address.rgBytes[5];
		//pass[1] = radioInfo.address.rgBytes[4];
		//pass[2] = radioInfo.address.rgBytes[3];
		//pass[3] = radioInfo.address.rgBytes[2];
		//pass[4] = radioInfo.address.rgBytes[1];
		//pass[5] = radioInfo.address.rgBytes[0];
	}
				...
				if (!wcscmp(btdi.szName, L"Nintendo RVL-WBC-01") || !wcscmp(btdi.szName, L"Nintendo RVL-CNT-01") || !wcscmp(btdi.szName, L"Nintendo RVL-CNT-01-TR"))
				{
					...
					if (!error)
					{
						if (pairVia12) {
							...
							pass[0] = btdi.Address.rgBytes[5];
							pass[1] = btdi.Address.rgBytes[4];
							pass[2] = btdi.Address.rgBytes[3];
							pass[3] = btdi.Address.rgBytes[2];
							pass[4] = btdi.Address.rgBytes[1];
							pass[5] = btdi.Address.rgBytes[0];
						}

						wprintf(_T("Connecting to %02x:%02x:%02x:%02x:%02x:%02x. 12Pairing %c. Password %04x:%04x:%04x:%04x:%04x:%04x\n"),
