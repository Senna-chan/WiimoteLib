...
// ------------------------------------------------------------------------------------
template <class T>
inline T sign(const T& val) { return (val < 0) ? T(-1) : T(1); }
	...
	while (SetupDiEnumDeviceInterfaces(dev_info, NULL, &guid, index, &didata))
	{
		...
		if (HidD_GetAttributes(Handle, &attrib))
		{
			...
			else
			{
				...
				while (!bStatusReceived && ((timeGetTime() - last_time) < 3000))
					Sleep(10);
				TRACE(_T(".. WriteFile() %s. 0x%08x"), bStatusReceived? _T("succeeded") :
					_T("failed"), GetLastError());
			}
			...
			if (!bStatusReceived && _HidD_SetOutputReport)
			{
				...
				while (!bStatusReceived && ((timeGetTime() - last_time) < 3000))
					Sleep(10);
				// did we get it?
				TRACE(_T(".. HID write %s. 0x%08x"), bStatusReceived? _T("succeeded") :
					_T("failed"), GetLastError());
			}
...
int wiimote::ParseInput(BYTE* buff)
{
	...
	default:
		DEEP_TRACE(_T(".. ** unknown input ** (happens). 0x%08x", type));
		///_ASSERT(0);
...
#define DEEP_TRACE_TYPE(type)	case OUT_##type: DEEP_TRACE(_T("WriteReport: ")\
	...
	if (!WriteFile(Handle, buff, REPORT_LENGTH, &written, &Overlapped))
	{
		...
		{
			TRACE(_T("We need to wait."));
			Sleep(1000);
			TRACE(_T("Sleepytime over."));
		}
	}
...
bool wiimote::EnableSpeaker(bool on)
{
	...
	if (on) TRACE(_T("enabling speaker."));
	else TRACE(_T("disabling speaker."));
...
#define READ_SIZE(ptr,size)	if(fread(ptr, size, 1, file) != 1) {		   \
	...
	while (1)
	{
		...
		else if (!strncmp(chunk_header.ckID, "data", 4))
		{
			...
			// and convert them
			bool res = Convert16bitMonoSamples(samples, false, total_samples, freq,
			                                   out);
			delete[] samples;
			return res;
		}
